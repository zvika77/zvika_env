...
SELECT /*+label(zvika_cross_channel_influence_chart)*/ source1 AS source1, SUM(assist_value)::FLOAT AS assist_value, SUM(closer_value)::FLOAT AS closer_value FROM ( SELECT /*+label(zvika_cross_channel_influence_chart)*/ '~' AS DATE, '~' AS site_name, source1 AS source1, sum(assist_value) as assist_value, sum(closer_value) as closer_value FROM ( select assist_source1 as source1, v_site_name, site_name, date, sum(case when assist_source1 <> closer_source1 then assist_value else 0 end) as assist_value, sum(case when assist_source1 = closer_source1 then assist_value else 0 end) closer_value from dash_assist_sources where client = 'nfl2' -- whitelist filter
  AND DATE BETWEEN '2014-01-01' AND '2014-02-01'
 -- site names          -- event filters
 AND model IN ('Regression','-') -- model filters
group by 1,2,3,4  ) q1 GROUP BY   1,2,3 -- ,4,5 
...
SELECT /*+label(zvika_cross_channel_chart)*/ source1 AS source1, SUM(clicks)::INT AS clicks, SUM(value)::FLOAT AS value, SUM(frac_contribution)::FLOAT AS frac_contribution, SUM(cost)::FLOAT AS cost, SUM(clicks_past)::INT AS clicks_past, SUM(value_past)::FLOAT AS value_past, SUM(frac_contribution_past)::FLOAT AS frac_contribution_past, SUM(cost_past)::FLOAT AS cost_past, SUM(VALUE)::FLOAT / SUM(clicks)::INT AS revenue_per_click, SUM(VALUE_past)::FLOAT / SUM(clicks_past)::INT AS revenue_per_click_past FROM ( SELECT /*+label(zvika_cross_channel_chart)*/ '~' AS DATE, site_name, source1, SUM(clicks) AS clicks, SUM(impressions) AS impressions, SUM(VALUE) AS VALUE, SUM(frac_contribution) AS frac_contribution, SUM(cost) AS cost, SUM(clicks_past) AS clicks_past, SUM(impressions_past) AS impressions_past, SUM(VALUE_past) AS value_past, SUM(frac_contribution_past) AS frac_contribution_past, SUM(cost_past) AS cost_past FROM ( SELECT model, DATE, v_site_id as site_id, v_site_name AS site_name, source1, SUM(impressions) AS impressions, SUM(clicks) AS clicks, SUM(VALUE) AS VALUE, SUM(frac_contribution) AS frac_contribution, 0.0 AS cost, 0 AS impressions_past, 0 AS clicks_past, 0.0 AS VALUE_past, 0.0 AS frac_contribution_past, 0.0 AS cost_past FROM exp_dash_performance_optimization WHERE client = 'nfl2' -- whitelist filter
AND DATE BETWEEN '2013-01-01' AND '2013-12-31'
AND event_type NOT IN ('all events')
  -- site names
 -- event filters
AND model IN ('Cross Channel with Imp (Offline & Online)','-') -- model filters         
GROUP BY
 1, 2, 3, 4, 5  UNION ALL          SELECT
 '' model,
DATE,
site_id,
site_name,
...
AND DATE BETWEEN '2013-01-01' AND '2013-12-31'
  ...
  GROUP BY
 1, 2, 3, 4, 5
 HAVING SUM(cost) > 0  UNION ALL
 SELECT
model,
( '2013-01-01'::DATE- '11/28/2012'::DATE) + DATE as DATE,
...
client = 'nfl2'
     -- whitelist filter
AND DATE BETWEEN '11/28/2012' AND '11/27/2013'
AND event_type NOT IN ('all events')
  -- site names
 -- event filters
AND model IN ('Cross Channel with Imp (Offline & Online)','-') -- model filters
