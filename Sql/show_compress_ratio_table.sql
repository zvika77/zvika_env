...
\echo Check Parameters : schema and  table_name  
\echo Run Audit on the object (Table) q run_audit.sql <object_name>
\echo =========================================


select audit_start_timestamp::date,projection_schema,anchor_table_name,sum_GB as comp_sizeGB,size_GB as raw_sizeGB, (((sum_GB/size_GB)-1) *100)::number(10,2) as '%reduction',
(size_GB/sum_gb)::int||':1' as ratio 
from 
(select projection_schema,anchor_table_name,(Sum(used_bytes)/1024/1024/1024)::int as sum_GB from projection_storage where  projection_id in (
...
FROM projections where projection_schema = :1  and anchor_table_name =  :2
 ...
 where rnum =1 ) 
 group by projection_schema,anchor_table_name ) compress_data
 join 
