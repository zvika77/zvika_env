...
from (
select to_char(time,'dd/mm/yy hh24') as time_d,trunc(time,'HH24') as time_e,
projection_name,
-- mergeout
...
and (projection_name in (select projection_name from projections where projection_schema ilike :1 and anchor_table_name ilike :2) or  node_name ilike :2)
group by time_d,time_e,projection_name,plan_type 
order by time_e desc ) a 
group by time_d,time_e,projection_name
order by 1 desc ;
